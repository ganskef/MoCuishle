...
/** TODO An HTTP request that came into the mock web server. */
class Request
@JvmOverloads
constructor(
  ...
  /**
   * TODO The failure MockWebServer recorded when attempting to decode this request. If, for
   * example, the inbound request was truncated, this exception will be non-null.
   */
  ...
  init {
    ...
    if (requestLine.isNotEmpty()) {
      ...
      // TODO validate "middleString", ip6, allow null
      val middleString = requestLine.substring(methodEnd + 1, pathEnd)
      if (method == "CONNECT") {
        this.requestUrl = "https://$middleString".toHttpUrlOrNull()
        this.path = null
      } else if (middleString.length > 4 &&
          middleString.substring(0, 4).equals("http", ignoreCase = true)
      ) {
        this.requestUrl = middleString.toHttpUrlOrNull()
        val pathBegin = middleString.indexOf('/', 8)
        this.path = if (pathBegin == -1) "/" else middleString.substring(pathBegin)
      } else {
        ...
        this.requestUrl = "$scheme://$hostname:$localPort$middleString".toHttpUrlOrNull()
        val pathBegin = middleString.indexOf('/')
        this.path = if (pathBegin == -1) "/" else middleString.substring(pathBegin)
      }
